...
class Solution {
    public int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
        ...
        int currCustomers = 0;
        int currProfit = 0;
        while(currCustomers > 0 || rotations < customers.length) {
            if(rotations < customers.length) currCustomers += customers[rotations];
            rotations++;
            
            int boardingCustomers = Math.min(currCustomers, 4); //put max number of customers on board
            currCustomers -= boardingCustomers;
            
            currProfit += boardingCustomers*boardingCost - runningCost;
