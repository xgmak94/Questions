...
class Solution {
    public int distanceBetweenBusStops(int[] distance, int a, int b) {
        ...
        for(int i = 0 ; i < distance.length ; i++) {
            if(i >= start && i < dest) {
                sumDest += distance[i]; //get distance going clockwise
            }
            total += distance[i]; //get total distance
        }
        return Math.min(sumDest, total - sumDest); //min of clockwise and counterclockwise
    }
}
